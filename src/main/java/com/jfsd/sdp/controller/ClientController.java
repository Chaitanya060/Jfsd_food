...
import com.jfsd.sdp.service.FoodItemService;
...
//	}
	...
	// Restaurant Begins
	  @GetMapping("/restaurant")
	   ...
	   public ModelAndView insertproductdemo(HttpServletRequest request,@RequestParam("fooditemimage") MultipartFile file) throws IOException, SerialException, SQLException
	   {
	     String msg = null;
...
//	   from her we wii start foodItems code
	   ...
	   public ModelAndView addfoditemdemo()
	   {
		   ModelAndView mv = new ModelAndView("addfooditem");
		   return mv;
	   }
	   
	   @PostMapping("insertfooditem")
	   public ModelAndView insertfooditemdemo(HttpServletRequest request,@RequestParam("image") MultipartFile file) throws IOException, SerialException, SQLException
	   {
		   String msg = null;
		   ...
		   Double cost = Double.valueOf(request.getParameter("cost"));
			  ...
			  f.setCost(cost);
			  f.setFooditemimage(blob);
			  
			  msg=foodItemService.AddFoodItem(f);
			  System.out.println(msg);
			  mv.setViewName("fooditemmsg");
			  mv.addObject("message",msg);
		   }	  
		   catch(Exception e)
		   {
			   msg = e.getMessage();
			   System.out.println(msg.toString());
			   mv.setViewName("fooditemerror");
			   mv.addObject("message",msg);
		   }
		   return mv;
	   }
	   
	   @GetMapping("viewallfooditem")
	   public ModelAndView viewallfooditemdemo()
	   {
		   List<FoodItem> fooditemlist = foodItemService.ViewAllFoodItems();
		   
		   ModelAndView mv = new ModelAndView("viewallfooditem");
		   
		   mv.addObject("fooditemlist", fooditemlist);
		   
		   return mv;
	   }
	   
	   @GetMapping("displayfooditemimage")
	   public ResponseEntity<byte[]> displayfooditemimagedemo(@RequestParam("id") int id) throws IOException, SQLException
	   {
	     FoodItem foodItem =  foodItemService.ViewFoodItemByID(id);
	     byte [] imageBytes = null;
	     imageBytes = foodItem.getFooditemimage().getBytes(1,(int) foodItem.getFooditemimage().length());

	     return ResponseEntity.ok().contentType(MediaType.IMAGE_JPEG).body(imageBytes);
	   }
	   
	   @GetMapping("viewfooditembyid")
	   public ModelAndView  viewfooditembyiddemo()
	   {
	   	   List<FoodItem> fooditemlist=foodItemService.ViewAllFoodItems();
	   	   ModelAndView mv=new ModelAndView("viewfooditembyid");
	   	   mv.addObject("productlist",fooditemlist);
	   	   return mv;
	   }
	   
	   
	   @PostMapping("displayfooditem")
	   public ModelAndView displayfooditemdemo(HttpServletRequest request)
	   {
		   int fid=Integer.parseInt(request.getParameter("fid"));
		   FoodItem foodItem=foodItemService.ViewFoodItemByID(fid);
		   ModelAndView mv=new ModelAndView("fooditemproduct");
		   mv.addObject("fooditem",foodItem);
		   return mv;
	   }
	   @GetMapping("viewfooditembycategory")
	   public ModelAndView viewfooditemByCategory()
	   {
		   ModelAndView mv=new ModelAndView("viewfooditembycategory");
		   return mv;
	   }
	   @PostMapping("displayfooditembycategory")
	   public ModelAndView viewfooditemByCategory(HttpServletRequest request)
	   {
		   String category=request.getParameter("category");
		   List<FoodItem> fooditemlist=foodItemService.viewAllFoodItemByCategory(category);
		   ModelAndView mv=new ModelAndView("displayfooditembycategory");
		   mv.addObject("fooditemlist",fooditemlist);
		   return mv;
	   }
